...
# ----- Theme (Black background + Cyan text) -----
BLACK = "#000000"
BLACK_2 = "#0B0B0B"
CYAN = "#00E5FF"
CYAN_2 = "#B8F8FF"
...
pio.templates["black_cyan"] = PLOTLY_TEMPLATE
pio.templates.default = "black_cyan"
...
# ----- App -----
app = Dash(__name__, title="Superstore Dashboard (Black/Cyan)")
...
<html>
  <head>
    ...
    <style>
      ...
      .card {{
        background: linear-gradient(180deg, rgba(11,11,11,0.98), rgba(0,0,0,0.98));
        border: 1px solid rgba(30,90,102,0.70);
        border-radius: 14px;
        padding: 14px 14px;
      ...
      .DateInput_input {{
        ...
        border: 1px solid rgba(30,90,102,0.70) !important;
      }}
      ...
      .preset-row label {{
        ...
        font-size: 12px;
        padding: 6px 10px;
        border: 1px solid rgba(30,90,102,0.65);
        border-radius: 999px;
...
def make_table(table_id: str) -> dash_table.DataTable:
    return dash_table.DataTable(
        ...
        style_table={"backgroundColor": "transparent", "borderRadius": "12px", "overflow": "hidden"},
    )
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                ...
                                html.Div(
                                    className="subtitle",
                                    children="Black background • Cyan text • Trendline • Top states/customers/products • YTD & YoY KPIs",
                                ),
                            ],
                        ),
                        ...
                        # UPDATED: Top products as PIE chart (instead of pivot table)
                        html.Div(
                            ...
                            children=[
                                html.Div(
                                    className="card",
                                    children=[
                                        dcc.Graph(id="fig-products-top", config={"displayModeBar": False}),
                                    ],
                                )
                            ],
                        ),
...
@app.callback(
    ...
    Output("fig-products-top", "figure"),
    Input("date-range", "start_date"),
    Input("date-range", "end_date"),
...
def update_dashboard(start_date, end_date, regions, categories, segments):
    ...
    fig_region = px.pie(by_region, names="Region", values="Sales", hole=0.45, title="Sales by Region", template="black_cyan")
    ...
    fig_category = px.bar(by_cat, x="Category", y="Sales", title="Sales by Category", template="black_cyan")
    ...
    # NEW: Top 10 Products as Pie Chart
    by_product = (
    ...
    fig_products.update_traces(
        marker=dict(colors=shade_list(len(by_product))),
        textfont_color=CYAN,
        textposition="inside",
    )
    fig_products.update_layout(height=360, margin=dict(l=18, r=18, t=54, b=18))
